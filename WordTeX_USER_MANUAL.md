...
7. [WordTeX Studio (Browser Tool)](#7-wordtex-studio-browser-tool)
...
WordTeX turns a Word manuscript into a LaTeX project that compiles, and lets the author check and correct the result without knowing LaTeX. It is written for the situation most small conferences and starting journals are in:
...
WordTeX closes that loop. Conversion runs entirely in the browser. Every recognised block — headings, paragraphs, **equations as editable LaTeX**, tables, figures, references — is shown next to a template-formatted preview; the author fixes what the converter got wrong through ordinary text editing and a block-type drop-down; a checklist names what still needs attention; and the exported project **always compiles**, because anything WordTeX cannot convert (legacy equation-editor objects, native Word charts, metafile images) becomes a visible placeholder at the right position instead of a build error.
...
- **Zero-install browser front end** — `wordtex_studio.html`, one 1.6 MB file that runs offline with no server, no upload and no network access
...
| **`scripts/headless_run.js`** | Editorial platforms, batch conversion, CI | `node scripts/headless_run.js wordtex_studio.html paper.docx out/` — same code path, no browser |
...
| Engine | **XeLaTeX** — `pdflatex` fails on the Unicode symbols WordTeX passes through; the bundled `latexmkrc` selects XeLaTeX automatically |
...
No installation. Download `wordtex_studio.html` from the latest GitHub release and double-click it, or open it from the browser's File menu.
...
|---|---|---|
| Windows | `WordTeX-autocompile.bat` + `wordtex-autocompile.ps1` | Keep both in one folder, double-click the `.bat`; confirm the watched folder once (default *Downloads*). A ZIP or folder can also be dropped onto the `.bat`. |
| macOS / Linux | `wordtex-autocompile.sh` | `./wordtex-autocompile.sh` (watches `~/Downloads`) or `./wordtex-autocompile.sh paper-LaTeX-Project.zip` |
...
| Argument | Default |
|---|---|
| `app.html` | `wordtex_studio.html` next to the repository root |
...
```
node scripts/build.js [outfile]        # default: wordtex_studio.html
```
...
```javascript
const I18N = require("./src/i18n.js");        // window.WordTeXI18N in the browser
I18N.setLanguage("zh");                       // "en" | "zh"; persisted in localStorage["wordtex-lang"]
...
## 7. WordTeX Studio (Browser Tool)

### 7.1 Opening the Tool

Double-click `wordtex_studio.html`, or open it from the browser's File menu. No installation, no server, no command line, and no network connection is required. **All processing happens locally in the browser; nothing is uploaded anywhere** — the page header carries a permanent reminder to that effect.
...
| Interface language | `localStorage` key `wordtex-lang` | until cleared; English when absent |
...
The 39 equations of m01 were also converted with Pandoc. After normalisation the two agree in 29 cases; the remaining 10 are runs the author set upright in Word, which WordTeX keeps as `\mathrm{}` and Pandoc ignores.
...
If you use WordTeX in published work, please cite both the software and the accompanying SoftwareX article once it is published. Machine-readable metadata is in `CITATION.cff`.
...
Continuous integration runs the suite on Ubuntu, Windows and macOS across Node 18, 20 and 22, builds `wordtex_studio.html` as an artifact, and compiles the example project with XeLaTeX, failing on any LaTeX error.
...
```
wordtolatexZL/
├── wordtex_studio.html         # single-file offline app (build output)
├── src/
