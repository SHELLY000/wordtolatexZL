#!/bin/sh
# WordTeX · macOS/Linux 本机自动编译：监视一个文件夹，*-LaTeX-Project*.zip 落地即用 XeLaTeX 编译并打开 PDF。
#   ./wordtex-autocompile.sh [watch-dir]          默认监视 ~/Downloads
#   ./wordtex-autocompile.sh some-LaTeX-Project.zip   只编译这一个
# 需要 xelatex（TeX Live / MacTeX）；有 latexmk 更好。
set -u
WATCH="${1:-$HOME/Downloads}"
BUILD_ROOT="${TMPDIR:-/tmp}/wordtex-build"
mkdir -p "$BUILD_ROOT"
