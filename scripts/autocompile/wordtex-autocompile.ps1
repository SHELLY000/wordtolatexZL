# WordTeX · Windows 本机自动编译脚本（监视下载目录，ZIP 落地即用 XeLaTeX 编译并打开 PDF）
# 用法：
...
$Host.UI.RawUI.WindowTitle = "WordTeX 自动编译（XeLaTeX）"
...
function Invoke-Compile([string]$zipPath) {
    ...
    # 编译目录用纯 ASCII 路径（C:\Users\Public\wordtex-build），避免中文路径让 TeX 出问题
    $buildRoot = Join-Path $env:PUBLIC "wordtex-build"
...
# ---- 选择要监视的文件夹：上次的选择保存在脚本旁边的 wordtex-autocompile.cfg 里 ----
$cfgPath = Join-Path $PSScriptRoot "wordtex-autocompile.cfg"
...
Save-Dir $watchDir
$Host.UI.RawUI.WindowTitle = "WordTeX 自动编译 · 监视 " + $watchDir
